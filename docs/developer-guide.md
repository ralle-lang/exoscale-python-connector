...
ruff check .                # lint
ruff format --check .       # formatting (separate CI gate from lint)
mypy src                    # type-check
```
...
- The generated skill bundle (`src/exoscale_connector/_skill/`) is in
  `[tool.ruff] extend-exclude`. It is generated by
  `scripts/generate_llms_txt.py`, and ruff 0.16+ formats Python code blocks
  inside Markdown — without the exclude it would rewrite `reference.md` and
  desync it from its generator.
