...
ruff format --check src tests scripts    # formatting (separate CI gate)
mypy src                                 # type-check
```
...
- **`ruff` is pinned exactly** (`ruff==0.16.1` in the `dev` extra) — the only
  ...
  Dependabot PRs; if a new version reformats the tree, that reformat belongs in
  the bump PR and nowhere else. Re-run `pip install -e ".[dev]"` after pulling a
  bump so your local ruff matches CI.
- **The ruff commands are path-scoped on purpose** — `src tests scripts`, exactly
  what `.github/workflows/ci.yml` runs. Ruff 0.16+ formats Python code blocks
  inside Markdown, so a bare `ruff format .` also rewrites the prose docs
  (`docs/`, `README.md`), which no gate checks. Scoping keeps what you run
  locally identical to what CI enforces.
- **Everything the bundle gate touches is in `[tool.ruff] extend-exclude`** — both
  its outputs and its inputs. `scripts/generate_llms_txt.py --check` is a CI gate
  that byte-compares the generated bundle, so ruff reformatting Markdown anywhere
  in that chain desyncs it and reddens the gate:
  - `src/exoscale_connector/_skill/` and `.claude/skills/exoscale-connector/` —
    the two copies the generator writes (wheel-shipped and repo-local).
  - `docs/asset-types/` — the pages the generator embeds *verbatim*. Reformatting
    an input changes what the generator would emit, which desyncs the committed
    bundle just as surely as editing an output.

  Consequence worth knowing: asset-type pages are never auto-formatted, so keep
  their Python snippets tidy by hand.
